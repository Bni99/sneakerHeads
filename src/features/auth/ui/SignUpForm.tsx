...
const SignUpForm = ({ open, onOpenChange }: SignUpFormProps) => {
  ...
  return (
    <>
      <Dialog.Root open={open} onOpenChange={onOpenChange}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 w-[23vw] -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white p-6 shadow-lg">
            ...
            <h4 className="text-brand-dark-grey mt-4 text-center">
              Already have an account ?{" "}
              <span
                className="text-brand-orange cursor-pointer font-medium"
                onClick={() => {
                  setIsLoginOpen(true);
                  onOpenChange(false);
                }}
      ...
      <SignInForm open={isLoginOpen} onOpenChange={setIsLoginOpen} />
    </>
  );
};
